...
export default function CheckinAmadasPage() {
  ...
  // Fecha de apertura de ingresos para AMADAS: 13 de Agosto de 2026
  const OPEN_DATE = new Date("2026-08-13T00:00:00");
  const isBeforeOpenDate = new Date() < OPEN_DATE;

  useEffect(() => {
    const auth = safeSessionStorage.getItem("comuarica_amadas_staff_auth");
    if (auth === "true") {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordInput.toLowerCase() === "staff2026") {
      setIsAuthenticated(true);
      safeSessionStorage.setItem("comuarica_amadas_staff_auth", "true");
      startPolling();
    } else {
  ...
  const handleLogout = () => {
    safeSessionStorage.removeItem("comuarica_amadas_staff_auth");
  ...
  const fetchAttendees = async () => {
    if (syncingId) return;
    try {
      const res = await fetch("/api/attendees-amadas");
      ...
      {!isAuthenticated ? (
        <div className="login-card">
          ...
          <form onSubmit={handleLogin}>
            ...
            {loginError && <p style={{ color: "#ef4444", marginTop: "12px", fontSize: "14px" }}>{loginError}</p>}
          </form>
        </div>
      ) : (
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <header style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px', alignItems: 'center' }}>
            <h1>Panel de Control Staff - AMADAS 2026</h1>
          ...
          <div className="stats-grid">
            ...
            <div className="stat-card">
              <h3 style={{ fontSize: '13px', opacity: 0.7 }}>INGRESADOS (AGOSTO 13)</h3>
          ...
          {isBeforeOpenDate && (
            <div style={{ background: 'rgba(234, 179, 8, 0.1)', border: '1px solid #eab308', padding: '15px', borderRadius: '10px', marginBottom: '30px', color: '#eab308', textAlign: 'center', fontSize: '14px' }}>
              ⚠️ El sistema de <strong>Ingreso</strong> se habilitará el <strong>13 de Agosto de 2026</strong>. Por ahora solo puedes gestionar confirmaciones de correo.
            </div>
          )}
              ...
              <div key={attendee.id} className="attendee-card">
                ...
                <div className="actions-group">
                  ...
                  <button 
                    ...
                    title={isBeforeOpenDate ? "Habilitado el 13 de Agosto" : ""}
                  >
